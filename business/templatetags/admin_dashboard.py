...
def get_daily_stats():
    ...
    daily_items = (
        Item.objects.filter(created_at__gte=start_date)
    ...
    stats_dict = {
        item["date"].strftime("%Y-%m-%d"): item["count"] for item in daily_items
    }
...
def get_category_stats():
    ...
    category_counts = (
        Item.objects.values("category").annotate(count=Count("id")).order_by("-count")
    )
...
def get_company_daily_stats(user):
    ...
    daily_items = (
        Item.objects.filter(company=company, created_at__gte=start_date)
        .annotate(date=TruncDay("created_at"))
        .values("date")
        .annotate(count=Count("id"))
        .order_by("date")
