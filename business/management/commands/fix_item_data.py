...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "UPDATE business_item SET price = '0' WHERE price = '' OR price IS NULL"
            )

            # Fix compare_at_price (set to NULL)
            cursor.execute(
                "UPDATE business_item SET compare_at_price = NULL WHERE compare_at_price = ''"
            )

            # Fix shipping_weight (set to NULL)
            cursor.execute(
                "UPDATE business_item SET shipping_weight = NULL WHERE shipping_weight = ''"
            )
