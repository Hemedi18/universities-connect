...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        if kwargs["delete_all"]:
            self.stdout.write(
                "Deleting ALL company data from source and target tables..."
            )
        ...
        with connection.cursor() as cursor:
            ...
            for source, target in tables_map:
                try:
                    ...
                    cursor.execute(
                        f"SELECT name FROM sqlite_master WHERE type='table' AND name='{source}';"
                    )
                    if not cursor.fetchone():
                        self.stdout.write(
                            self.style.WARNING(
                                f"Source table {source} does not exist. Skipping."
                            )
                        )
                        continue
                    ...
                    cursor.execute(
                        f"SELECT name FROM sqlite_master WHERE type='table' AND name='{target}';"
                    )
                    if not cursor.fetchone():
                        self.stdout.write(
                            self.style.WARNING(
                                f"Target table {target} does not exist. Skipping."
                            )
                        )
                        continue
                    ...
                    if target == "company_company":
                        ...
                        cursor.execute(
                            f"INSERT INTO {target} SELECT * FROM {source} WHERE user_id IN (SELECT id FROM auth_user)"
                        )
                    elif target in ["company_review", "company_report"]:
                        # Only copy reviews/reports where both user and company exist
                        cursor.execute(
                            f"INSERT INTO {target} SELECT * FROM {source} WHERE user_id IN (SELECT id FROM auth_user) AND company_id IN (SELECT id FROM company_company)"
                        )
                    else:
                    ...
                    if target == "company_company":
                        self.stdout.write(
                            "Fixing empty strings in foreign keys for company_company..."
                        )
                        cursor.execute(
                            f"UPDATE {target} SET region_id = NULL WHERE region_id = '';"
                        )
                        cursor.execute(
                            f"UPDATE {target} SET district_id = NULL WHERE district_id = '';"
                        )

                        # Ensure no orphaned companies exist (fixes IntegrityError on user_id)
                        self.stdout.write("Removing orphaned company records...")
                        cursor.execute(
                            f"DELETE FROM {target} WHERE user_id NOT IN (SELECT id FROM auth_user)"
                        )

                    if target in ["company_review", "company_report"]:
                        self.stdout.write(f"Removing orphaned {target} records...")
                        cursor.execute(
                            f"DELETE FROM {target} WHERE company_id NOT IN (SELECT id FROM company_company)"
                        )

                    self.stdout.write(
                        self.style.SUCCESS(f"Successfully copied data to {target}.")
                    )

                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"Error copying {source} to {target}: {e}")
                    )
            ...
            try:
                ...
                if src_exists and tgt_exists:
                    ...
                    self.stdout.write(f"Copying data from {source} to {target}...")
                    cursor.execute(
                        f"INSERT INTO {target} (company_id, user_id) SELECT company_id, user_id FROM {source} WHERE company_id IN (SELECT id FROM company_company) AND user_id IN (SELECT id FROM auth_user)"
                    )
                    self.stdout.write(
                        self.style.SUCCESS(f"Successfully copied data to {target}.")
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Manual data migration finished. You can now run 'python manage.py migrate'."
            )
        )
