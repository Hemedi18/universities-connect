...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        user, created = User.objects.get_or_create(
            username=company_data["username"], email=company_data["email"]
        )
        ...
        products = [
            (
                "iPhone 15 Pro Max",
                "Apple flagship phone, Titanium, 256GB",
                3500000,
                "Electronics",
            ),
            (
                "Samsung Galaxy S24 Ultra",
                "AI Phone, 200MP Camera, S-Pen",
                3200000,
                "Electronics",
            ),
            (
                "MacBook Pro 16 M3 Max",
                "Space Black, 32GB RAM, 1TB SSD",
                7500000,
                "Electronics",
            ),
            (
                "Dell XPS 13 Plus",
                "OLED Touch, Core i7, 16GB RAM",
                4500000,
                "Electronics",
            ),
            (
            ...
            (
                "iPad Pro 12.9 M2",
                "Liquid Retina XDR, 256GB, WiFi",
                3200000,
                "Electronics",
            ),
            (
                "Canon EOS R5",
                "8K Video, 45MP Full Frame Mirrorless",
                9000000,
                "Electronics",
            ),
            (
            ...
            (
                "Apple Watch Ultra 2",
                "Rugged Titanium Case, Alpine Loop",
                2200000,
                "Electronics",
            ),
            (
                "Samsung Odyssey G9",
                "49-inch Curved Gaming Monitor",
                3800000,
                "Electronics",
            ),
            ...
            (
                ...
                "7-inch Screen, White Joy-Cons",
                950000,
                "Electronics",
            ),
            (
                "GoPro Hero 12 Black",
                "Action Camera, 5.3K Video",
                1100000,
                "Electronics",
            ),
            (
                "DJI Mini 4 Pro",
                "4K HDR Video Drone, Fly More Combo",
                2800000,
                "Electronics",
            ),
            (
                "Kindle Paperwhite",
                "16GB, 6.8-inch Display, Waterproof",
                450000,
                "Electronics",
            ),
            ...
            (
                "Google Pixel 8 Pro",
                "Android 14, 50MP Camera, Obsidian",
                2500000,
                "Electronics",
            ),
            (
            ...
            (
                ...
                "Business Ultrabook, Carbon Fiber",
                4800000,
                "Electronics",
            ),
        ...
        self.stdout.write(
            f"Adding {len(products)} items with real images (this may take a moment)..."
        )
            ...
            if not Item.objects.filter(seller=user, title=title).exists():
                ...
                try:
                    ...
                    with urllib.request.urlopen(
                        ...
                        item.image.save(
                            file_name, ContentFile(response.read()), save=True
                        )
                        self.stdout.write(f" [OK] Added: {title}")
                except Exception as e:
                    self.stdout.write(
                        self.style.WARNING(f" [FAIL] Image for {title}: {e}")
                    )
                    # Fallback to placeholder text image
                    try:
                        safe_title = urllib.parse.quote(title)
                        fallback_url = (
                            f"https://placehold.co/600x400/png?text={safe_title}"
                        )
                        req = urllib.request.Request(
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f" [FAIL] Fallback also failed for {title}: {e2}"
                            )
                        )
            else:
